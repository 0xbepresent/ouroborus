"""
Common utility functions for Ouroborus.
...
from src.utils.exceptions import OuroborusError, CodeQLError
...
def read_file(file_name: str) -> str:
    ...
    Raises:
        OuroborusError: If file cannot be read (not found, permission denied, encoding error).
    """
    try:
        with Path(file_name).open("r", encoding="utf-8") as f:
            return f.read()
    except FileNotFoundError as e:
        raise OuroborusError(f"File not found: {file_name}") from e
    except PermissionError as e:
        raise OuroborusError(f"Permission denied reading file: {file_name}") from e
    except UnicodeDecodeError as e:
        raise OuroborusError(f"Failed to decode file as UTF-8: {file_name}") from e
    except OSError as e:
        raise OuroborusError(f"OS error while reading file: {file_name}") from e
...
def write_file_text(file_name: str, data: str) -> None:
    ...
    Raises:
        OuroborusError: If file cannot be written (permission denied, disk full, etc.).
    """
    try:
        with Path(file_name).open("w", encoding="utf-8") as f:
            f.write(data)
    except PermissionError as e:
        raise OuroborusError(f"Permission denied writing file: {file_name}") from e
    except OSError as e:
        raise OuroborusError(f"OS error while writing file: {file_name}") from e
...
def read_yml(file_path: str) -> Dict[str, Any]:
    ...
    Raises:
        OuroborusError: If file cannot be read or YAML parsing fails.
    """
    try:
        with Path(file_path).open('r', encoding="utf-8") as file:
            return yaml.safe_load(file)
    except FileNotFoundError as e:
        raise OuroborusError(f"YAML file not found: {file_path}") from e
    except PermissionError as e:
        raise OuroborusError(f"Permission denied reading YAML file: {file_path}") from e
    except yaml.YAMLError as e:
        raise OuroborusError(f"Failed to parse YAML file: {file_path}") from e
    except OSError as e:
        raise OuroborusError(f"OS error while reading YAML file: {file_path}") from e