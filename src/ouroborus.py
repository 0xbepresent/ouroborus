#!/usr/bin/env python3
"""
Core analysis engine for Ouroborus (C/CodeQL path; used for compatibility).
...
from src.utils.exceptions import OuroborusError, CodeQLError, LLMApiError
...
class IssueAnalyzer:
    ...
    def build_prompt_by_template(
        ...
        Raises:
            OuroborusError: If template files cannot be read (not found, permission denied, etc.).
        """
    ...
    def ensure_directories_exist(self, dirs: List[str]) -> None:
        ...
        Raises:
            OuroborusError: If directory creation fails (permission denied, etc.).
        """
        for d in dirs:
            dir_path = Path(d)
            if not dir_path.exists():
                try:
                    ...
                    raise OuroborusError(f"Permission denied creating directory: {d}") from e
                except OSError as e:
                    raise OuroborusError(f"OS error creating directory: {d}") from e
    ...
    def save_raw_input_data(
        ...
        Raises:
            OuroborusError: If file cannot be written (permission denied, etc.).
        """
        raw_data = json.dumps({
    ...
    def process_issue_type(
        ...
        Raises:
            ...
            OuroborusError: If result files cannot be written.
    ...
    def run(self, dbs_dir: str) -> None:
        ...
        Raises:
            ...
            OuroborusError: If directory creation or file writing fails.
